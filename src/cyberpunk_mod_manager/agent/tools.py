...
async def install_local_mod(mod_id: int, archive_name: str) -> str:
    ...
    return await mod_ops.install_from_archive(mod_id, archive_path)


async def preview_install_plan(mod_id: int, archive_name: str = "") -> str:
    """检查压缩包结构并生成安装计划（不执行安装）。

    Args:
        mod_id: Nexus Mods 模组数字 ID。
        archive_name: 可选，本地压缩包路径；省略则从 downloads 查找。

    Returns:
        JSON：file_mappings、plan_items、plan_source、inspection 等。
    """
    await mod_ops.ensure_mod_in_inventory(mod_id)
    if archive_name:
        path = Path(archive_name)
        ...
        archive_path = mod_ops.resolve_local_archive_path(mod_id) or mod_ops.find_local_archive(
            mod_id
        )
    if archive_path is None:
        return _error_json(f"未找到模组 {mod_id} 的本地压缩包")
...
async def uninstall_mod_with_plan_review(mod_id: int, force: bool = False) -> str:
    """读取卸载计划、LLM 解读后按记录执行卸载。
    ...
    Returns:
        JSON：plan_explanation、removed_files_count 等。
    """
    ...
    explanation = await explain_uninstall_plan(
        mod_id,
        plan.to_dict(),
        mod_name=mod_name,
    )
...
async def fetch_trending_mods() -> str:
    """获取 Nexus 热门模组列表（无参数）。"""
    mods = await discovery.fetch_trending_mods()
    return json.dumps({"count": len(mods), "mods": mods}, ensure_ascii=False)


async def sync_tracked_mods() -> str:
    """将 Nexus 账户追踪的模组同步到本地库存（无参数）。"""
    result = await discovery.sync_tracked_mods_to_inventory()
    return json.dumps(result, ensure_ascii=False)


async def fetch_updated_mod_feed(period: str = "1w") -> str:
    """获取 Nexus 近期有活动的模组 feed。

    Args:
        period: 时间窗口，如 1d / 1w / 1m。
    """
    feed = await discovery.fetch_updated_feed(period=period)
    hits = discovery.compare_local_with_updated_feed(feed)
    return json.dumps(
        {
            "count": len(feed),
            "period": period,
...
async def batch_mod_status(mod_ids: list[int]) -> str:
    """批量查询模组在 Nexus 上的可用性与状态。

    Args:
        mod_ids: Nexus 模组 ID 数组。
    """
    rows = await discovery.batch_mod_availability(mod_ids)
    return json.dumps({"count": len(rows), "mods": rows}, ensure_ascii=False)
...
def build_toolkit() -> Toolkit:
    return Toolkit(
        tools=[
            ...
            ModFunctionTool(fetch_trending_mods),
            ModFunctionTool(sync_tracked_mods),
            ModFunctionTool(fetch_updated_mod_feed),
            ModFunctionTool(batch_mod_status),
